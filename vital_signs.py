...
def main():
    ...
    for languagecode in wikilanguagecodes:
        ...
        editor_metrics_dump_iterator(languagecode)
    ...
    # VITAL SIGNS
    for languagecode in wikilanguagecodes:
        vital_signs_db_iterator(languagecode)
        print(languagecode)
...
def create_vital_signs_editors_db():
    ...
    conn = sqlite3.connect(databases_path + vital_signs_editors_db)
    cursor = conn.cursor()
    conn2 = sqlite3.connect(databases_path + vital_signs_web_db)
...
def get_mediawiki_paths(languagecode):
    ...
    base_path = "./mediawiki_history_dumps/"

    # Percorso del file all-time
    dumps_path = f'{base_path}/{languagecode}wiki/{cym}.{languagecode}wiki.all-time.tsv.bz2'
    if os.path.isfile(dumps_path):
        ...
        for year in range(1999, cy):
            dumps_path = f'{base_path}/{languagecode}wiki/{year}.tsv.bz2'
            if os.path.isfile(dumps_path):
                d_paths.append(dumps_path)
        ...
        if not d_paths:
            for year in range(1999, cy):
                for month in range(1, 13):
                    ...
                    dumps_path = f'{base_path}/{languagecode}wiki/{year}-{month_str}.tsv.bz2'
                    if os.path.isfile(dumps_path):
                        d_paths.append(dumps_path)
...
'''
def get_mediawiki_paths_old(languagecode):
    ...
    if os.path.isdir('/public/dumps/public/other/mediawiki_history/'+cym) == False:
        cym = datetime.datetime.strptime(
            cym, '%Y-%m')-dateutil.relativedelta.relativedelta(months=1)
        cym = cym.strftime('%Y-%m')
        print('/public/dumps/public/other/mediawiki_history/'+cym)

    dumps_path = '/public/dumps/public/other/mediawiki_history/'+cym+'/' + \
        languagecode+'wiki/'+cym+'.'+languagecode+'wiki.all-time.tsv.bz2'
    if os.path.isfile(dumps_path):
        ...
        for year in range(1999, 2025):
            dumps_path = '/public/dumps/public/other/mediawiki_history/'+cym+'/' + \
                languagecode+'wiki/'+cym+'.'+languagecode + \
                'wiki.'+str(year)+'.tsv.bz2'
            if os.path.isfile(dumps_path):
                d_paths.append(dumps_path)
        ...
        if len(d_paths) == 0:
            for year in range(1999, 2025):  # months
                for month in range(0, 13):
                    if month > 9:
                        dumps_path = '/public/dumps/public/other/mediawiki_history/'+cym+'/'+languagecode + \
                            'wiki/'+cym+'.'+languagecode+'wiki.' + \
                            str(year)+'-'+str(month)+'.tsv.bz2'
                    else:
                        dumps_path = '/public/dumps/public/other/mediawiki_history/'+cym+'/'+languagecode + \
                            'wiki/'+cym+'.'+languagecode+'wiki.' + \
                            str(year)+'-0'+str(month)+'.tsv.bz2'
...
def editor_metrics_primary_language_calculation():
    ...
    conn = sqlite3.connect(databases_path + 'vital_signs_editors.db')
    cursor = conn.cursor()
...
# this prints both the output to a file and to the terminal screen.
...
if __name__ == '__main__':
    ...
    def get_cleaned_subdirectories(path):
    ...
    path = "./mediawiki_history_dumps/"
    wikilanguagecodes = get_cleaned_subdirectories(path)
    print(wikilanguagecodes)

    wikipedialanguage_numberarticles = []

    print('* Starting the COMMUNITY HEALTH METRICS '+cycle_year_month +
          ' at this exact time: ' + str(datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")))
    main()

    finishTime = time.time()
    print('* Done with the COMMUNITY HEALTH METRICS completed successfuly after: ' +
          str(datetime.timedelta(seconds=finishTime - startTime)))

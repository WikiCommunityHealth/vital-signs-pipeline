...
def download_dumps():
    def get_wiki_directories(base_url):
        ...
        directories = [urljoin(base_url, a['href']) for a in soup.find_all(
            'a', href=True) if a['href'].endswith('wiki/')]
        return directories
...
def test_download_dumps():
    ...
    url = "https://dumps.wikimedia.org/other/mediawiki_history/2025-04/aawiki/"

    os.makedirs(config.dumps_path, exist_ok=True)

    
    wiki_name = url.rstrip('/').split('/')[-1]
    wiki_save_path = os.path.join(config.dumps_path, wiki_name)
    os.makedirs(wiki_save_path, exist_ok=True)
